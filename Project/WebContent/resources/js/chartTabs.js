...
$(function () {
	...
	var dialog = $("#dialog").dialog({
		...
		buttons: {
			[create]: function () {
				...
				var architecture_dropdown = $("#organizationalPortalArchitectureDropdown").val();
				if (tab_title != "" && architecture_dropdown != null) {
					addTab();
					$(this).dialog("close");
					$.ajax({
						url: "getPropertiesDictionaryArabic",
					...
					$(".alert_message").html('<div class="alert alert-warning" role="alert">' + all_fields_are_required + '</div>');
	...
	function addTab() {
		...
		if (count <= 9) {
			var label = tabTitle.val() || "Tab " + tabCounter,
				...
				li = $(tabTemplate.replace(/#\{href\}/g, "#" + id).replace(/#\{label\}/g, label)),
				tabContentHtml = tabContent.val() || "Tab " + tabCounter + " content.";
			let getArchitectureValue = $("#organizationalPortalArchitectureDropdown").val();
			$.ajax({
				url: "addTabIntoDatabase", // url: "/findAttributes?dbName="+db_name,
				type: "POST",
				data: { 'tab_title': label, 'containerid': id, 'architectureValue': getArchitectureValue },
				dataType: "json",
				beforeSend: function () {
				},
				success: function (data) {
	...
	$("#tabs").tabs({
		activate: function (event, ui) {
			...
			console.log("tab activated currentTabId", currentTabId)
			// add AJAX request
			$.ajax({
				url: "getCurrentDivArchitectureId",
				async: false,
				data: { 'tabId': currentTabId },
				type: "POST",
				success: function (architectureId) {
					currentArchitectureId = architectureId[0];
					const myvalue = currentArchitectureId.toString(); // Replace with the ID you want to check
					for (let i = 0; i < architectureData.length; i++) {
						const item = architectureData[i];
						if (item["id"] === myvalue) {
							//					    $(".alert_message").html('<div class="alert alert-info" role="alert">'+item["architecture_name"]+'</div>');
							//					    $(".showArchitectureTitle").html('<div class="alert alert-info" role="alert">'+item["architecture_name"]+'</div>');
							//						$("#element_title").html('<span>'+associated_architecture+': '+item["architecture_name"]+'</span>');
							$("#element_title").html('<span>' + associated_architecture_cap + ': ' + item["architecture_name"] + '</span>');
						}
					}
					// use the data returned by the server to generate charts
				},
		...
		if (chartMultipleData.attributeValues[0].includes("Very High") || chartMultipleData.attributeValues[0].includes("High") || chartMultipleData.attributeValues[0].includes("Medium") || chartMultipleData.attributeValues[0].includes("Low") || chartMultipleData.attributeValues[0].includes("Very Low")) {
			...
			*/
			...
			if (checkArrayValue === false) {
				...
				let x_axis_label_in_arabic = "Ù…Ù…ÙŠØ²Ø§Øª " + chart_title1;
				...
				var options = {
					...
					yaxis: {
						...
						title: {
							text: getLanguage == "En" ? 'Number of ' + element_name : customDictionary[element_name.replaceAll(" ", "_")] + ' Ø¹Ø¯Ø¯ ',
						},
						lines: {
							show: true
						}
	...
	if (checkArrayValue === false) {
		...
		var options = {
			...
			yaxis: {
				...
				opposite: getLanguage == "En" ? false : true,
				title: {
					text: getLanguage == "En" ? 'Millions' : 'Ù…Ù„Ø§ÙŠÙŠÙ†',
				},
				lines: {
					show: true
				},
			...
			title: {
				//			text : 'Traffic Sources',
				//				text : getLanguage == "En" ? "Budget Utilization" : "Ø§Ø³ØªØ®Ø¯Ø§Ù… Ø§Ù„Ù…ÙŠØ²Ø§Ù†ÙŠØ©",
				//				text : getLanguage == "En" ? element : element,
	...
	if (chartMultipleData != undefined) {
		if (chartMultipleData.attributeValues[0].includes("Very High") || chartMultipleData.attributeValues[0].includes("High") || chartMultipleData.attributeValues[0].includes("Medium") || chartMultipleData.attributeValues[0].includes("Low") || chartMultipleData.attributeValues[0].includes("Very Low")) {
			...
			*/
				...
				var options = {
					...
					xaxis: {
						...
						title: {
							...
							//							text : getLanguage == "En" ? element_name : chart_title1,
						},
						lines: {
							show: true
						},
					...
					yaxis: {
						...
						opposite: getLanguage == "En" ? false : true,
						title: {
							text: getLanguage == "En" ? 'Average' : y_axis_arabic_title,
						},
						lines: {
							show: true
						}
					...
					title: {
						...
						//						text : getLanguage == "En" ? element_name+" "+'Criticality' : chart_title_in_arabic,
						align: 'center',
					},
					grid: {
						position: 'front'
			...
			let getAverageArray = [];
			let sumOfIteration = [];
			console.log("chartMultipleData", chartMultipleData);
			for (var i = 0; i <= chartMultipleData.nameRow.length - 1; i++) {
				let pushIterationData = [];
				let sumNumber = 0;
				for (var j = 0; j <= enumArray.length - 1; j++) {
			...
			let InitialArray = []
			let NoneArray = []
			const flatArray = chartMultipleData.attributeValues.flat();
			const uniqueValues = Array.from(new Set(flatArray));
			var newArrraaay = [];
			var arrayOfNumbers = sumOfIteration.map(Number);
			let zerosArray = []
			...
			for (var i = 0; i <= uniqueValues.length - 1; i++) {
				if (i == 0) {
					newArrraaay.push({ name: uniqueValues[i], data: arrayOfNumbers })
				}
				else {
					newArrraaay.push({ name: uniqueValues[i], data: zerosArray })
				}
			}
			...
			*/
			...
			if (checkArrayValue === false) {
				...
				console.log("OptimizedArray", OptimizedArray)
				console.log("ManagedArray", ManagedArray)
				console.log("FunctionalArray", FunctionalArray)
				console.log("ChallengedArray", ChallengedArray)
		...
		if (checkArrayValue === false) {
			...
			$("#" + saveButton).click(function () {
				...
				$.ajax({
					...
					type: "POST",
					data: JSON.stringify(graphDataJson),
					//							data: {graphDataJson: JSON.stringify(graphDataJson), architectureId: currentArchitectureId},
					dataType: 'json',
					async: 'true',
					success: function (data) {
						if (data == '1') {
		...
		if (checkArrayValue === false) {
			...
			var options = {
				...
				title: {
					//text : 'Traffic Sources',
					//			text: attribute,
					//					text: getLanguage == "En" ? element_name : "ØªÙˆØ²ÙŠØ¹ Ø§Ù„Ù…ÙŠØ²Ø§Ù†ÙŠØ©",
					//					text: getLanguage == "En" ? element_name+" "+attributesTextArea: element_nameelement_name+" "+attributesTextArea,
					align: 'center',
				},
...
function drawChartFromDB(data, containerID) {
	...
	if (data.chart_type == "clusteredBar" && (data.chart_Operation == "Count" || data.chart_Operation == "count")) {
		...
		//				title : {
		////					text : 'Application Component',
		//					text: getLanguage == "En" ? x_axis_label: chart_title1+" Ù…Ù…ÙŠØ²Ø§Øª",
		//				},
		//				lines : {
		//					show : true
		//				},
	...
	if (data.chart_Operation == "Distribution" && data.chart_type == "pie") {
		...
		//			title : {
		////				text : 'Traffic Sources',
		//				text: getLanguage == "En" ? data.x_axis_label:'ØªÙˆØ²ÙŠØ¹ Ø§Ù„Ù…ÙŠØ²Ø§Ù†ÙŠØ©',
		//				align : 'center',
		//			},
...
function enableChartSaveButton(id, saveButton) {
	...
	console.log("makeChartContrainerID", makeChartContrainerID);

	$.ajax({
		url: "checkChartStatus", // url: "/findTables?dbName="+db_name,
		//type: "GET",
		data: { 'chartDivName': makeChartContrainerID },
		async: false,
		success: function (data) {
			console.log("data", data);
			if (data != '') {
				let checkIfChartExist = $("#" + makeChartContrainerID).attr("style");
				console.log("checkIfChartExist", checkIfChartExist)
				...
				$(".alert_message").html(`<div class="alert alert-warning" role="alert">${save_chart_first}</div>`);
		...
		if (checkArrayValue === false) {
			...
			var options = {
				...
				title: {
					...
					//			text: attribute,
					//					text: getLanguage == "En" ? element_name : "ØªÙˆØ²ÙŠØ¹ Ø§Ù„Ù…ÙŠØ²Ø§Ù†ÙŠØ©",
					//					text: getLanguage == "En" ? element_name+" "+attributesTextArea : element_name+" "+attributesTextArea,
					align: 'center',
				},
				labels: chartData.nameRow,
				responsive: [{
		...
		if (chartMultipleData.attributeValues[0].includes("Very High") || chartMultipleData.attributeValues[0].includes("High") || chartMultipleData.attributeValues[0].includes("Medium") || chartMultipleData.attributeValues[0].includes("Low") || chartMultipleData.attributeValues[0].includes("Very Low")) {
			...
			*/
			...
			if (checkArrayValue === false) {
				...
				var chart_title_in_arabic = chart_title2 + " " + chart_title1;
				let x_axis_label = element + " Characteristics";
				x_axis_label_in_arabic = "Ù…Ù…ÙŠØ²Ø§Øª " + chart_title1;
				let y_axis_title = attribute;
				//	var yAxisData = [];
				//	var yAxisLabel = [];
				var categoriesArray = [];
...
$(document).ready(function () {
	$.ajax({
		...
		success: function (data) {
			...
			console.log("data", data)
			for (var i = 0; i <= data.length - 1; i++) {
				$("#organizationalPortalArchitectureDropdown").append(`<option value="${data[i]["id"]}">${data[i]["architecture_name"]}</option>`);
			}
		}
	}),
