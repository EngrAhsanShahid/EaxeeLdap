...
function relationship_modal(objectsArray,idd,maxPossibleObjects,elementIcon, save_btn_id,maxValues){
	objectsAdded = [];
	brCodes = {};
	$.each(elementIcon, function(i, val) {
		let temp = val.split('---');
		brCodes[temp[0].replaceAll(" ","_")] = temp[2]; 
	});
	console.log('brCodes',brCodes)
	//$('#table_data').empty();
...
//	elementIcon = "Application Component";
	...
	let rowIdsList = document.getElementById(idd).getElementsByClassName("object_id_form");
...
//		$.each( object_names_form, function( key, value ) {
		for(var i=0;i<loopLength;i++){
			...
			let objID = rowIdsList[i].innerHTML;
			elementIcon =elementIconList[i].innerHTML;
//			let table = $('#example').DataTable();
			objectsAdded.push(value);
			$('#example').DataTable().row.add([
			  '<td class="elementIcon">'+elementIcon+'</td>','<td class="object_name">'+value+'</td>','<td><span class="hiddenRowIDs" hidden="hidden">'+objID+'</span><a style="cursor:pointer" onclick="removeRow(\''+idd+'_'+value.replaceAll(/[&\/\\#+()`$"\""~%'\'';,.:*?<>{}]/g, '_').replaceAll(" ", "_")+'\')">&#10006;</a></td>'
			])
...
//	createList(idd);
	...
	$('#addButtonRel').append('<button onclick="add_object(\''+idd+'\', \''+save_btn_id+'\', \''+maxPossibleObjects+'\');" class="form-control">'+save_changes+'</button>')
...
function add_object(idd, save_btn_id, maxPossibleObjects){
	...
	let hiddenRowIDs = document.getElementById("example").getElementsByClassName("hiddenRowIDs");
	var tag_input = "";
//	$('.table_data').empty();
	// console.log('idd', idd);
...
//		$.each( object_names, function( key, value ) {
		for(var i=0;i<loopLength;i++){
			...
			elementIcon =elementIconList[i].innerHTML;
			rowID = hiddenRowIDs[i].innerHTML;
			$(inputField).append('<tr><td class="elementIcon" style=\"width: 5px !important\">'+elementIcon+'</td><td class="object_name_form">'+tag_input+'</td><td hidden="hidden" class="object_id_form">'+rowID+'</td></tr>');
...
//		});
		...
		if(maxPossibleObjects == '1'){
			
		}else if(maxPossibleObjects == '2'){
			for(var j=0;j<2-loopLength;j++){
				$(inputField).append('<tr><td class = \"first_td elementIcon\"></td><td class=\"object_name_form\"></td></tr>');
			}
			
		}else if(maxPossibleObjects == '3'){
			for(var j=0;j<3-loopLength;j++){
				$(inputField).append('<tr><td class = \"first_td elementIcon\"></td><td class=\"object_name_form\"></td></tr>');
			}
...
function add_into_modal_table(tag_id,object_name, elementIcon,elementClass,objID){
	let relationPossible = false;
	$.ajax({
		url: "checkRelationshipLimitFromDatabaseEntries",
		type: 'GET',
		data: {
			'objID': objID,
			'elementName': elementClass,
			'brcode': brCodes[elementClass]
		},
	...
	if (!objectsAdded.includes(object_name) && relationPossible == false) {
		$(".tree_modal_alert_message").html('<div class="alert alert-danger" role="alert"> ' + relation_limit_exceeded + '</div>');
		...
		if (length_check > elementMaxNumber) {
			...
			if (!findObject) {
				...
				let table = $('#example').DataTable();
				$('#example').DataTable().row.add([
					'<td class="elementIcon"><img style=\"width:20px; height:14px\" draggable=\"false\" src=\"resources/js/examples/editors/images/images1/' + elementIcon + '\" ></td>', '<td class="object_name ' + elementClass + '">' + object_name + '</td>', '<td><span class="hiddenRowIDs" hidden="hidden">' + objID + '</span><a style="cursor:pointer" onclick="removeRow(\'' + div_idd + '_' + object_name.replaceAll(" ", "_") + '\')">&#10006;</a></td>'
