var firstName = [];
function renameNavigation(graphID, liID, folderNameID, navigationName, parentFolderName, type, parentId, architectureId, architectureName) {
	console.log("Inside renameNavigation")
	console.log("graphID,liID,folderNameID,navigationName,parentFolderName,type, parentId, architectureId, architectureName")
	console.log(graphID, liID, folderNameID, navigationName, parentFolderName, type, parentId, architectureId, architectureName)
					...
					[Change]: function updateText() {
						...
						$.ajax({
							...
							success: function (data) {
								refreshTree();
								getGraphNames();
								drawNavigationFromDB(changeTextValue, graphID, 'role_admin', parentFolderName, architectureId, architectureName, parentId);
								//					   $("#"+navigationName+"11" ).remove();
								console.log("navigationName_folderId", navigationName + "_" + parentId)
								$("#" + navigationName).remove();
								$("#" + navigationName + "_" + parentId + "11").remove();
								historyNumberCloseDiv(navigationName + "_" + parentId)
								//							$("#"+liID).remove();
