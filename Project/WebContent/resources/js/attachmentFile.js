var attachmentTable;
var attachmentTableCreated = false;
	...
	// $("#recycle_bin_list_for_attachments").append('<li id="'+folderID+'2" class="model_list '+folderName+'" style="padding-left:38px" oncontextmenu="on_context_menu_restore_attachment(\''+id+'\', \''+folderID+'\', \''+folderName+'\',\''+ulID+'\',\''+type+'\',\''+company_name+'\',\''+parentID+'\');"><i class="fa fa-folder" aria-hidden="true"></i>&nbsp;'+folderName.replaceAll('---', ' ')+'</li>')
 		...
 		$.ajax({
		      ...
		      success:function(data) {
				...
				// thisFolderSpan.setAttribute('oncontextmenu', "on_context_menu_restore_attachment('"+id+"', '"+folderID+"', '"+folderName+"','"+ulID+"','"+type+"','"+company_name+"','"+parentID+"')");
...
function on_context_menu_remove_attachment(graphID,liID, folderName,modelName, parentFolder){
			...
			if(thisAttachment.classList.contains("deletedObject")){
...
//		  content_ += '<a style="text-decoration:none; cursor:pointer;" onclick="renameDiagram(\''+graphID+'\', \''+liID+'\', \''+folderName+'\', \''+modelName+'\', \''+parentFolder+'\')"><div class="item"><i class="fa fa-edit" aria-hidden="true"></i><spring:message code="rename"/></div></a>';	
		  content_ += '<a style="text-decoration:none; cursor:pointer;" onclick="checkFileExist(\''+modelName.replaceAll("---"," ")+'\')"><div class="item"><i class="fa fa-download" aria-hidden="true"></i>'+download+'</div></a>';
...
function removeAttachment(graphID, liID, folderName, modelName1, parentFolder) {
	...
	thisObject.setAttribute('oncontextmenu', 'on_context_menu_restore_attachment("' + graphID + '", "' + modelName1 + '", "' + folderName + '", "' + liID + '", "' + parentFolder + '")');
...
// 	Restore Attachment
	function on_context_menu_restore_attachment(graphID, name, folderName, liID,type,company_name,parentID){
			...
			var thisAttachment = document.getElementById(liID)
			if(thisAttachment.classList.contains("deletedObjectParent")){
				console.log("found class deletedObject")
				return
			}
		...
		// var tree = '';
  		$.ajax({
		      ...
		      success:function(data) {
				...
				// thisAttachment.setAttribute('oncontextmenu', 'on_context_menu_attachment("'+company_name+'", "'+name+'", "'+graphID+'", "'+folderName+'", "'+parentID+'", "'+company_name+parentID+'", "folder")>');
					...
					thisFolderSpan.setAttribute('oncontextmenu', 'on_context_menu_attachment("'+company_name+'", "'+name+'", "'+graphID+'", "'+folderName+'", "'+parentID+'", "'+company_name+parentID+'", "folder")');
		  ...
		  alert_message_function();
	  		...
	  		$.ajax({
			      ...
			      success:function() {
			    	...
			    	var thisAttachment = document.getElementById(liID)
					thisAttachment.classList.remove("deletedObject")
					thisAttachment.setAttribute('oncontextmenu', 'on_context_menu_remove_attachment("'+graphID+'", "'+liID+'", "'+folderName+'", "'+name.replaceAll(" ","---")+'", "'+type+'")');
			      }	  	
			});
		}
	}
...
function checkFileExist(fileName){
	$.ajax({
		url: "checkFileExistOrNot", // url: "/findTables?dbName="+db_name,
		type: "POST",
		data:{
			'fileName' :fileName
		},	      
		success: function(data) {
			...
			if(data == 'Exists'){
		    	...
		    	$(".alert_message").html('<div class="alert alert-danger" role="alert">'+file_not_exist+'</div>');
		    	alert_message_function();			    		
			}
		},
	    error: function(jqXHR, textStatus, errorThrown) {
	        ...
	        try {
	            ...
	            // If the error response is not JSON, log the error message
...
function attachment_modal(divId,saveBtnId,treeData){
	...
	if(attachmentTableCreated == false){
		attachmentTable = $('#attachmentExample').DataTable({paging: false});
		attachmentTableCreated = true;
	}
	attachmentTable
	    .clear()
	    .draw();
...
//	else{
		...
		if(!findObject){
			let iteration = i+1;
			let attachmentTable = $('#attachmentExample').DataTable();
			$('#attachmentExample').DataTable().row.add([
...
function removeAttachmentRow(trId){
	...
	var attachmentTable = $('#attachmentExample').DataTable();
	 
	var rows = attachmentTable
	    .rows("#"+trId)
	    .remove()
	    .draw()
...
//			elementIcon =elementIconList[i].innerHTML;
			$(inputField).append('<tr><td>'+iteration+'</td><td class="attachment_name_form">'+tag_input+'</td><td><i class="fa fa-download" aria-hidden="true" onclick="checkFileExist(\''+tag_input+'\')"></i></td><td class="delAttachmentRow"><i class="fa fa-trash" aria-hidden="true" onclick="delAttachmentRow(\''+save_btn_id+'\');"></i></td></tr>');
