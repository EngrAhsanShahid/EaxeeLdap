var firstName = [];
function renameRoadmap(roadmapid, liID, folderName_ID, roadmapName, companyName, parentID, parentFolder, folderID, architectureID, architectureName) {
	// console.log("Inside renameRoadmap")
	...
	//}
	//else{
	// let previousName = roadmapName.replace(/---/g, ' ');
	$("#change_text").val(roadmapName.replace(/---/g, ' '));
	//}
	// }
	// else {
					...
					[Change]: function updateText() {
						...
						$.ajax({
							...
							success: function (data) {
								...
								let roadMapLiId = changeTextValue.replaceAll(" ", "---")+""+roadmapid;
								refreshTree(roadMapLiId);
								getGraphNames();
								getRoadmap(changeTextValue, roadmapid, companyName, parentID, folderID, architectureID, architectureName);
								$("#" + roadmapid + "11").remove();
								$("#" + roadmapName).remove();
								$("#" + roadmapName + "_" + parentID + "11").remove();
								historyNumberCloseDiv(roadmapName + "_" + parentID)
								//							$("#"+liID).remove();
